...
uniform int divisionsLightRay = 8;
...
vec3 skyColor(vec3 dir, vec3 sunDir, vec3 origin, float dist) {
	...
	if(moonColor.x == 0.0 && moonColor.y == 0.0 && moonColor.z == 0.0){
		return moonColor;
	}
	...
	vec3 tau = fourPI * (betaR * opticalDepthRayleigh + 1.1 * betaM * opticalDepthMie);

	vec3 result = moonColor * exp(-tau);
...
void main() {
	...
	if (cameraMode == REGULAR) { // normal camera
		...
		result = skyColor(dir, dir, origin, dist);
	}
